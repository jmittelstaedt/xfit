...
from typing import Callable, Mapping, Optional, Sequence, Tuple, Union, Hashable
...
def plot_fits(
    ...
    **kwargs
    ...
    xselections = {
        dim: sel for dim, sel in selections.items() if dim in fit_ds.xda.dims
    }
    ...
    yselections = {
        dim: sel for dim, sel in selections.items() if dim in fit_ds.yda.dims
    }
    ...
    pselections = {
        ...
        if dim in fit_ds.popt.dims and dim != "param"
    }
    pomissions = {
        dim: sel
        for dim, sel in omissions.items()
        if dim in fit_ds.popt.dims and dim != "param"
    }
    pranges = {
        dim: sel
        for dim, sel in ranges.items()
        if dim in fit_ds.popt.dims and dim != "param"
    }

    xda = da_filter(
        fit_ds.xda, selections=xselections, omissions=xomissions, ranges=xranges
    )
    yda = da_filter(
        fit_ds.yda, selections=yselections, omissions=yomissions, ranges=yranges
    )
    pda = da_filter(
        fit_ds.popt, selections=pselections, omissions=pomissions, ranges=pranges
    )

    if not np.all(np.isnan(fit_ds.yerr_da)):
        yeda = da_filter(
            fit_ds.yerr_da, selections=yselections, omissions=yomissions, ranges=yranges
        )
    else:
        yeda = None
    ...
    dims_with_many_values = [
        dim for dim in pda.dims if len(pda[dim]) > 1 and dim != "param"
    ]
...
def plot_model_fits(
    ...
    **kwargs,
    ...
    if plot_models == "all":
        plot_models = list(
            x for x in fit_ds.attrs["models"].keys() if x not in background_models
        )
    ...
    for m in plot_models + background_models:
        if m not in fit_ds.attrs["models"]:
            raise ValueError(
                f"{m} is not a model of this system! Included models are: {fit_ds.attrs['models'].keys()}"
            )
    ...
    for combo in coord_combos:
        ...
        all_params = {
            pn: float(pda.sel(**selection_dict, param=pn).values)
            for pn in fit_ds.attrs["param_names"]
        }
        ...
        if plot_total:
            ...
            plt.plot(
                fit_dom, fit_ds.attrs["fit_func"](fit_dom, *modparams), label="total"
            )
