...
bs = xr.DataArray(
    np.linspace(0, 10, 6), coords={"b_true": np.linspace(0, 10, 6)}, dims="b_true"
)
xs = xr.DataArray(
    np.linspace(0, 10, 11), coords={"x": np.linspace(0, 10, 11)}, dims="x"
)

data = xr.DataArray(
    np.ones((1, 10)) * np.arange(5).reshape(5, 1),
    dims=["b_true", "x"],
    coords={"b_true": np.arange(5), "x": np.arange(10)},
)

data_ds = xr.Dataset({"data": data})

expected_popt = xr.DataArray(
    np.arange(5).reshape(1, 5),
    coords={"param": ["b"], "b_true": np.arange(5)},
    dims=["param", "b_true"],
)

expected_perr = xr.DataArray(
    np.zeros((1, 5)),
    coords={"param": ["b"], "b_true": np.arange(5)},
    dims=["param", "b_true"],
)

expected_pcov = xr.DataArray(
    np.zeros((1, 1, 5)),
